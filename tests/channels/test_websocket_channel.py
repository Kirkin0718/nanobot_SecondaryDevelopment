...
@pytest.mark.asyncio
async def test_webui_hidden_history_skips_transcript_but_marks_inbound(
    bus: MagicMock,
) -> None:
    ...
    msg = bus.publish_inbound.await_args.args[0]
    assert msg.content.startswith("【教练·生成笔记】")
    assert msg.metadata[HIDDEN_HISTORY_META] == {"kind": "webui_hidden"}
    assert read_transcript_lines("websocket:chat-hidden") == []
