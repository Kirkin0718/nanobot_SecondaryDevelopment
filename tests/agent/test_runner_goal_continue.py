...
By default ``max_goal_continues=0`` so a plain-text yield waits for the user
(no wait-spam). Explicit positive caps still allow autonomous looping for tests.
"""
...
@pytest.mark.asyncio
async def test_runner_waits_for_user_by_default_when_goal_active():
    """Default max_goal_continues=0: active goal must not spam after a yield."""
    from nanobot.agent.runner import AgentRunner

    provider = MagicMock(spec=LLMProvider)
    provider.chat_with_retry = AsyncMock(return_value=LLMResponse(
        content="waiting for your choice", tool_calls=[], usage={},
    ))
    tools = MagicMock()
    tools.get_definitions.return_value = []
    ...
    assert result.stop_reason == "completed"
    assert result.final_content == "waiting for your choice"
...
@pytest.mark.asyncio
async def test_runner_forces_continue_when_goal_active_and_cap_allows():
    """With an explicit continue budget, plain-text finals may re-enter the loop."""
    from nanobot.agent.runner import AgentRunner
...
async def test_runner_goal_continue_not_limited_by_injection_cycle_cap():
    ...
    result = await runner.run(make_run_spec(provider,
        ...
        max_tool_result_chars=_MAX_TOOL_RESULT_CHARS,
        goal_active_predicate=lambda: True,
        max_goal_continues=max_iterations,
...
@pytest.mark.asyncio
async def test_runner_skips_continue_when_callback_returns_none():
    """Callable continue message returning None means wait for the user."""
    from nanobot.agent.runner import AgentRunner

    provider = MagicMock(spec=LLMProvider)
    provider.chat_with_retry = AsyncMock(return_value=LLMResponse(
        content="please choose", tool_calls=[], usage={},
    ))
    tools = MagicMock()
    tools.get_definitions.return_value = []
