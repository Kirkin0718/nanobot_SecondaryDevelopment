...
def test_goal_awaiting_user_pauses_auto_continue():
    from nanobot.session.goal_state import (
        goal_awaiting_user,
    ...
    meta = {GOAL_STATE_KEY: {"status": "active", "objective": "Learn Java"}}
    assert sustained_goal_should_auto_continue(meta) is True
    assert goal_awaiting_user(meta) is False

    assert set_goal_awaiting_user(meta, True) is True
    assert goal_awaiting_user(meta) is True
    assert sustained_goal_should_auto_continue(meta) is False
    assert sustained_goal_active(meta) is True  # still active, just paused

    assert set_goal_awaiting_user(meta, True) is False  # no-op
    assert set_goal_awaiting_user(meta, False) is True
    assert goal_awaiting_user(meta) is False
    assert sustained_goal_should_auto_continue(meta) is True

    assert is_goal_idle_wait_filler("用户没有新输入，继续等待。不做任何操作。")
    assert is_goal_idle_wait_filler("请选择一个选项继续")
    assert not is_goal_idle_wait_filler("今天完成这三件事：读文档、写示例、打卡。")
