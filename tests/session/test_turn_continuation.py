...
@pytest.mark.asyncio
async def test_awaiting_user_blocks_maybe_continue_and_forces_finalize():
    """Continuation unavailable while goal_state.awaiting_user is True."""
    meta = {
        GOAL_STATE_KEY: {"status": "active", "objective": "pick", "awaiting_user": True},
    }
    ...
    ctx = SimpleNamespace(
        session=SimpleNamespace(metadata=meta),
        msg=InboundMessage(channel="cli", sender_id="u1", chat_id="c1", content="go"),
        session_key="cli:c1",
        pending_queue=asyncio.Queue(),
        stop_reason="max_iterations",
        final_content="choose",
        all_messages=[],
    )
...
@pytest.mark.asyncio
async def test_internal_continuation_skips_when_awaiting_user():
    meta = {
        GOAL_STATE_KEY: {
            "status": "active",
