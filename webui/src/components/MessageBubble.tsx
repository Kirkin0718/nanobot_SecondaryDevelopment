...
interface MessageBubbleProps {
  ...
  onForkFromHere?: () => void;
  /** Click handler for inline choice chips under assistant replies. */
  onChoiceSelect?: (label: string) => void;
  /** Tool-approval chips (同意/拒绝/终止) — must not start a new chat turn. */
  onToolApprovalSelect?: (approvalId: string, decision: "approve" | "deny" | "stop") => void;
}
        ...
        <>
          ...
          {!message.isStreaming
            ...
            <div
              ...
              role="group"
              aria-label={t("message.choiceGroupAria")}
            >
              {message.buttons.map((row, rowIndex) => (
                <div key={`choice-row-${rowIndex}`} className="flex flex-wrap gap-2">
                  {row.map((label) => (
                    <button
                      key={label}
                      type="button"
                      ...
                      onClick={() => {
                        if (choicesUsed) return;
                        const approvalId =
                          message.agentUi?.kind === "tool_approval"
                            ? message.agentUi.approval_id
                            : undefined;
                        if (approvalId && onToolApprovalSelect) {
                          const decision =
                            label === "同意修改"
                              ? "approve"
                              : label === "拒绝修改"
                          ...
                          if (decision) {
                            setChoicesUsed(true);
                            onToolApprovalSelect(approvalId, decision);
                            return;
                          }
                        }
                        setChoicesUsed(true);
                      ...
                      className={cn(
                        "rounded-full border border-border/80 bg-background px-3.5 py-1.5",
