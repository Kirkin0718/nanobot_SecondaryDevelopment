...
import { GoalProgressBar } from "@/components/coach/GoalProgressBar";
import { NotesDrawer } from "@/components/coach/NotesDrawer";
import { CheckinDrawer } from "@/components/coach/CheckinDrawer";
import { ProgressHubDrawer } from "@/components/coach/ProgressHubDrawer";
...
export function ThreadShell({
  ...
  const [notesPathOverride, setNotesPathOverride] = useState<string | null>(null);
  ...
  const hasUserOrAssistant = useMemo(
    () => displayMessages.some((m) => m.role === "user" || m.role === "assistant"),
    [displayMessages],
  );

  useEffect(() => {
    setNotesVisited(false);
    setNotesOpen(false);
    setCheckinOpen(false);
    setHubOpen(false);
    setNotesPathOverride(null);
  }, [historyKey]);

  const checkedInToday = !!coachState.coach?.checked_in_today;
  const notesHaveContent = Boolean(coachState.coach?.notes?.trim());
  ...
  const openNotesPathFromHub = useCallback((path: string) => {
    setHubOpen(false);
    setFilePreviewPath(null);
    setNotesPathOverride(path);
    setNotesOpen(true);
    setNotesVisited(true);
  }, []);
  ...
  const handleConfirmGoal = useCallback(() => {
    handleThreadSend(t("thread.coach.prompts.confirmGoal"));
  }, [handleThreadSend, t]);
    ...
    <>
      ...
      {session ? (
        <ThreadComposer
          ...
          goalState={goalState}
          hideGoalChrome={!!goalState?.active}
  ...
  return (
    <CoachProvider value={coachState}>
    <section ref={shellRef} className="relative flex min-h-0 flex-1 overflow-hidden">
      <div className="relative flex min-w-0 flex-1 flex-col overflow-hidden">
        {!hideHeader ? (
          <ThreadHeader
            ...
            onOpenNotes={historyKey ? openNotes : undefined}
            onOpenCheckin={historyKey ? openCheckin : undefined}
            onOpenHub={historyKey ? openHub : undefined}
            checkedInToday={checkedInToday}
          />
        ) : null}
        {historyKey ? (
          <div className="shrink-0 space-y-2 border-b border-border/40 px-3 pb-2 pt-1 sm:px-4">
            <JourneyStrip
              hasActiveGoal={!!goalState?.active}
              hasSession={!!session}
              hasMessages={hasUserOrAssistant}
              notesVisited={notesVisited}
              hasNotesContent={notesHaveContent}
              checkedInToday={checkedInToday}
              compactActions={!!goalState?.active}
              onConfirmGoal={handleConfirmGoal}
              onGoalChoice={(label) => handleThreadSend(label)}
              onFocusSessions={onToggleSidebar}
        ...
        <ThreadViewport
          ...
          onChoiceSelect={(label) => handleThreadSend(label)}
          onToolApprovalSelect={
            chatId
              ? (approvalId, decision) => {
                  client.sendToolApprovalResponse(chatId, approvalId, decision);
                }
              : undefined
          }
        />
      </div>
      {filePreviewPath && historyKey && !notesOpen ? (
        <FilePreviewPanel
      ...
      <NotesDrawer
        ...
        onOpenChange={(next) => {
          setNotesOpen(next);
          if (next) setNotesVisited(true);
          else setNotesPathOverride(null);
        }}
        messages={displayMessages}
        onSend={(content, options) => handleThreadSend(content, undefined, options)}
        disabled={isStreaming}
        notesPathOverride={notesPathOverride}
      />
      <CheckinDrawer
        open={checkinOpen}
      ...
      <ProgressHubDrawer
        ...
        sessionKey={historyKey}
        onOpenNotesPath={openNotesPathFromHub}
      />
    </section>
    </CoachProvider>
  );
