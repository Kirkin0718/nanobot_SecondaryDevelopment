...
describe("MessageBubble", () => {
  ...
  it("renders choice chips and sends the clicked label once", () => {
    const onChoiceSelect = vi.fn();
    const message: UIMessage = {
      id: "a-choices",
      role: "assistant",
      content: "How should we install?",
      createdAt: Date.now(),
      buttons: [["同意安装 JDK", "我自己装"]],
    };

    render(<MessageBubble message={message} onChoiceSelect={onChoiceSelect} />);

    fireEvent.click(screen.getByRole("button", { name: "同意安装 JDK" }));
    expect(onChoiceSelect).toHaveBeenCalledWith("同意安装 JDK");
    expect(onChoiceSelect).toHaveBeenCalledTimes(1);

    fireEvent.click(screen.getByRole("button", { name: "我自己装" }));
    expect(onChoiceSelect).toHaveBeenCalledTimes(1);
  });
});
