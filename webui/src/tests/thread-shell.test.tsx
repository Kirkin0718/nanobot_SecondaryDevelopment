...
describe("ThreadShell", () => {
  ...
  it("renders coach journey chrome and opens notes from the header", async () => {
    const client = makeClient();
    const coachPayload = {
      active: true,
      topic: "java-basics",
    ...
    vi.stubGlobal(
      ...
      vi.fn().mockImplementation(async (input: RequestInfo) => {
        const url = String(input);
        if (url.includes("/coach") && !url.includes("/hub") && !url.includes("/notes") && !url.includes("/checkin")) {
          return httpJson(coachPayload);
        }
        if (url.includes("/history") || url.includes("/transcript") || url.includes("/messages")) {
    ...
    render(wrap(
      client,
      <ThreadShell
        session={session("coach-journey")}
        title="Coach journey"
        onToggleSidebar={() => {}}
      />,
    ));

    expect(await screen.findByTestId("journey-strip")).toBeInTheDocument();
    expect(screen.getByTestId("thread-header-coach-actions")).toBeInTheDocument();

    await act(async () => {
      client._emitChat("coach-journey", {
        event: "goal_state",
        chat_id: "coach-journey",
        goal_state: {
          active: true,
          objective: "Learn Java",
          ui_summary: "Stage 1",
        },
      } as import("@/lib/types").InboundEvent);
    });

    expect(await screen.findByTestId("goal-progress-bar")).toBeInTheDocument();

    fireEvent.click(screen.getByRole("button", { name: "Open notes" }));
    expect(await screen.findByTestId("notes-drawer")).toBeInTheDocument();
  });
});
