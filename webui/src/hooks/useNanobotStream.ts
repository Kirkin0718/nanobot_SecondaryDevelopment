...
export interface SendOptions {
  ...
  /** Send to the agent without showing the user prompt in the chat UI. */
...
export function useNanobotStream(
      ...
      if (ev.event === "message") {
        ...
        if (sideChannelEvent) {
          setMessages((prev) => absorbCompleteAssistantMessage(prev, {
            ...
            ...(ev.buttons?.length ? { buttons: ev.buttons } : {}),
  ...
  const send = useCallback(
    (content: string, images?: SendImage[], options?: SendOptions) => {
      ...
      const hiddenHistory = options?.hiddenHistory === true;
      flushPendingStreamEvents();
      ...
      setMessages((prev) => {
        ...
        if (hiddenHistory) {
          return sideChannel ? base : pruneReasoningOnlyPlaceholders(base);
        }
        return [
