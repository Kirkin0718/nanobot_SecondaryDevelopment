...
export interface CoachStatePayload {
  ...
  progress: CoachProgressPayload;
  notes: string;
  log_tail: string;
  checked_in_today: boolean;
  /** ISO dates with a check-in in the recent window (typically last 7 days). */
  checkin_days?: string[];
  today: string;
}

export interface CoachHubTopic {
  topic: string;
  paths: { path: string; notes: string; log: string };
  progress: CoachProgressPayload;
...
export interface CoachHubOrphanNote {
  path: string;
  notes_preview: string;
  notes_bytes: number;
  updated_at: number;
...
export interface CoachHubPayload {
  topics: CoachHubTopic[];
  orphan_notes: CoachHubOrphanNote[];
  active_goals_summary: string;
  topic_count: number;
  notes_count: number;
}

function coachValuesHeader(values: Record<string, unknown>): HeadersInit {
  return { "X-Nanobot-Coach-Values": encodeURIComponent(JSON.stringify(values)) };
}
...
export async function fetchCoachState(
  ...
  key: string,
  base: string = "",
): Promise<CoachStatePayload> {
  return request<CoachStatePayload>(
    `${base}/api/sessions/${encodeURIComponent(key)}/coach`,
    token,
    undefined,
    API_READ_TIMEOUT_MS,
  );
...
export async function fetchCoachHub(
  ...
  key: string,
  base: string = "",
): Promise<CoachHubPayload> {
  return request<CoachHubPayload>(
    `${base}/api/sessions/${encodeURIComponent(key)}/coach/hub`,
    token,
    undefined,
    API_READ_TIMEOUT_MS,
  );
...
export async function saveCoachNotes(
  ...
  const values: Record<string, unknown> = { text };
  if (typeof path === "string" && path.trim()) values.path = path.trim();
  return request(
    `${base}/api/sessions/${encodeURIComponent(key)}/coach/notes`,
    token,
    { headers: coachValuesHeader(values) },
    API_READ_TIMEOUT_MS,
  );
}

export async function postCoachCheckin(
  token: string,
  key: string,
  text?: string,
  base: string = "",
): Promise<{ ok: boolean; path: string; today: string; entry: string }> {
  const values: Record<string, unknown> = {};
  if (typeof text === "string") values.text = text;
  return request(
    `${base}/api/sessions/${encodeURIComponent(key)}/coach/checkin`,
    token,
    Object.keys(values).length ? { headers: coachValuesHeader(values) } : undefined,
    API_READ_TIMEOUT_MS,
  );
}
