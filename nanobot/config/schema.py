...
class ToolsConfig(Base):
    ...
    approval: ToolApprovalConfig = Field(
        default_factory=lambda: _lazy_default(
            "nanobot.agent.tools.approval_config",
            "ToolApprovalConfig",
        ),
    )
    goal_await: GoalAwaitConfig = Field(
        default_factory=lambda: _lazy_default(
            "nanobot.agent.tools.goal_await_config",
            "GoalAwaitConfig",
        ),
        validation_alias=AliasChoices("goalAwait", "goal_await"),
        serialization_alias="goalAwait",
    )
...
def _resolve_tool_config_refs() -> None:
    ...
    mod.ToolApprovalConfig = ToolApprovalConfig  # type: ignore[attr-defined]
    mod.GoalAwaitConfig = GoalAwaitConfig  # type: ignore[attr-defined]
