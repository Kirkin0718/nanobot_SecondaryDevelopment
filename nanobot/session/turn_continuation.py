...
def _goal_continuation_available(
    ...
    *,
    ...
    if not sustained_goal_active(session_metadata):
        return False
    # User was asked to choose / reply — stay quiet until they do.
    if goal_awaiting_user(session_metadata):
        return False
    try:
...
def _goal_continuation_prompt(metadata: Mapping[str, Any] | None) -> str:
    ...
    goal = f"{chr(10).join(lines)}\n\n" if lines else ""
    return (
        f"{header}{goal}"
        "Continue productive tool work only when you can make progress without "
        "the user. Do not narrate idle waiting. Do not invent status lines about "
        "having no new user input. Do not send another question, reminder, or "
        "buttons. If you need the user, produce no user-visible text and stop."
    )
