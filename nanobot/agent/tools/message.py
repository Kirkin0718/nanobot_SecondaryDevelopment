...
class MessageTool(Tool):
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        # Already waiting on the user: stay silent (no wait-spam).
        if self._goal_is_awaiting_user():
            return (
                "Goal is awaiting user input; message not delivered. "
                "Stay silent until the user replies or picks a button."
            )
        ...
        try:
            ...
            # Pause auto-continuation after any delivered same-chat message while a
            # sustained goal is open (options / status), so we never re-ask / flood.
            has_buttons = bool(buttons and any(row for row in buttons))
            if (
                has_buttons
                or is_goal_idle_wait_filler(content)
                or self._goal_is_active()
            ):
                self._pause_goal_for_user_choice()
    ...
    def _goal_is_awaiting_user(self) -> bool:
        ...
        if request_ctx is None or not request_ctx.session_key:
            return False
        try:
            sess = self._sessions.get_or_create(request_ctx.session_key)
        except Exception:
            return False
        return goal_awaiting_user(sess.metadata)
    ...
    def _goal_is_active(self) -> bool:
        ...
        try:
            ...
            return False
        return sustained_goal_active(sess.metadata)

    def _pause_goal_for_user_choice(self) -> None:
        """Mark the active sustained goal as waiting so the runtime stays silent."""
        if self._sessions is None:
            return
        request_ctx = current_request_context()
        if request_ctx is None or not request_ctx.session_key:
        ...
        try:
            ...
            logger.exception("Failed to load session while pausing goal for user choice")
            return
        if set_goal_awaiting_user(sess.metadata, True):
            self._sessions.save(sess)
            logger.info(
                "Paused sustained goal auto-continue for session {} (awaiting user)",
