...
from nanobot.agent import tool_approval as tool_approval_mod
...
from nanobot.session.goal_state import (
    ...
    is_goal_idle_wait_filler,
    runner_wall_llm_timeout_s,
    set_goal_awaiting_user,
    sustained_goal_active,
    sustained_goal_should_auto_continue,
)
...
class AgentLoop:
    ...
    _TRANSITIONS: dict[tuple[TurnState, str], TurnState] = {
        ...
        # Internal continuation while the goal is paused for a user choice.
        (TurnState.RESTORE, "awaiting_user"): TurnState.DONE,
    ...
    def __init__(
        ...
        # Per-session timers that end silent wait-after-choice.
        self._goal_await_timers: dict[str, asyncio.Task] = {}
    ...
    def _persist_user_message_early(
        ...
        **kwargs: Any,
        ...
        if has_text or media_paths:
            ...
            hidden = (msg.metadata or {}).get(HIDDEN_HISTORY_META)
            if hidden is not None:
                extra[HIDDEN_HISTORY_META] = hidden
    ...
    def _goal_await_timeout_minutes(self) -> float:
        cfg = getattr(self.tools_config, "goal_await", None)
        try:
            minutes = float(getattr(cfg, "timeout_minutes", 3.0) if cfg is not None else 3.0)
        except (TypeError, ValueError):
            minutes = 3.0
        return minutes if minutes > 0 else 0.0

    def _cancel_goal_await_timer(self, session_key: str) -> None:
        task = self._goal_await_timers.pop(session_key, None)
        if task is not None and not task.done():
            task.cancel()

    def _arm_goal_await_timer(
        self,
        session_key: str,
        *,
        channel: str,
        ...
        """Start (or refresh) the silent-wait timeout for *session_key*."""
        minutes = self._goal_await_timeout_minutes()
        self._cancel_goal_await_timer(session_key)
        if minutes <= 0:
            return
        self._goal_await_timers[session_key] = asyncio.create_task(
            self._goal_await_timeout(
                session_key,
                channel=channel,
                chat_id=chat_id,
                minutes=minutes,
            ...
            name=f"goal-await-{session_key}",
        )

    async def _goal_await_timeout(
        self,
        session_key: str,
        *,
        channel: str,
        ...
        """End the waiting process after *minutes* with no user input."""
        try:
            await asyncio.sleep(max(0.0, minutes) * 60.0)
        except asyncio.CancelledError:
            return
        finally:
            current = self._goal_await_timers.get(session_key)
            if current is asyncio.current_task():
                self._goal_await_timers.pop(session_key, None)

        try:
            session = self.sessions.get_or_create(session_key)
        except Exception:
            logger.exception("Failed to load session {} on goal-await timeout", session_key)
            return
        if not goal_awaiting_user(session.metadata):
            return
        if set_goal_awaiting_user(session.metadata, False):
            self.sessions.save(session)

        cancelled = await self._cancel_active_tasks(session_key)
        pending = self._pending_queues.pop(session_key, None)
        ...
        label = f"{minutes:g}"
        notice = (
            f"已等待 {label} 分钟无操作，本次等待已结束。"
            "需要时直接回复或点选选项继续。"
        )
        logger.info(
            "Goal await timed out for {} after {} min (cancelled={})",
            session_key,
            label,
            cancelled,
        )
    ...
    async def _run_agent_loop(
        ...
        *on_stream_end(resuming)*: called when a streaming session finishes.
        ...
        def _goal_continue() -> str | None:
            if session is None:
                return None
            if not sustained_goal_should_auto_continue(session.metadata):
                return None
            _goal_lines = goal_state_runtime_lines(session.metadata)
            if not _goal_lines:
                return None
            return (
                "You have an active sustained goal:\n\n"
                + "\n".join(_goal_lines)
                + "\n\nThe user has not sent a new message. "
                "Do NOT narrate waiting or invent idle status lines. "
                "Do NOT install, search loops, or complete_goal. "
                "Do NOT send another question, reminder, or buttons. "
                "If you cannot make progress without the user, produce NO "
                "user-visible text and stop immediately — stay silent."
            )

        session_metadata = session.metadata if session is not None else None
        hook_metadata = dict(metadata or {})
        approval_cfg = self.tools_config.approval
        approval_registered = False
        if approval_cfg.enable and active_session_key:

            async def _publish_tool_approval(
                content: str,
                *,
                approval_id: str,
                tool_name: str,
            ...
            async def _cancel_from_approval() -> int:
                return await self._cancel_active_tasks(active_session_key)

            tool_approval_mod.REGISTRY.register_turn_bridge(
                active_session_key,
                publish=_publish_tool_approval,
                cancel_turn=_cancel_from_approval,
            )
            approval_registered = True
            hook_metadata["_tool_approval_config"] = approval_cfg
            hook_metadata["session_key"] = active_session_key
        try:
            ...
            result = await self.runner.run(AgentRunSpec(
                ...
                goal_active_predicate=lambda: (
                    sustained_goal_should_auto_continue(session.metadata)
                    if session is not None
                    else False
                ),
                goal_continue_message=_goal_continue,
                # Never auto-loop after a plain-text yield while a goal is open —
                # wait for the user's next message / chip click (prevents wait spam).
                max_goal_continues=0,
            ...
            if approval_registered and active_session_key:
                tool_approval_mod.REGISTRY.unregister_turn_bridge(active_session_key)
    ...
    def stop(self) -> None:
        """Stop the agent loop."""
        self._running = False
        for key in list(self._goal_await_timers):
            self._cancel_goal_await_timer(key)
    ...
    def _assemble_outbound(
        ...
        *,
        ...
        # Hard-stop idle wait spam while a sustained goal is open.
        if session is not None and is_goal_idle_wait_filler(final_content):
            if set_goal_awaiting_user(session.metadata, True):
                self.sessions.save(session)
            logger.info(
                "Suppressed idle-wait filler for {}:{}",
                msg.channel,
                msg.sender_id,
            )
            return None
        # Drop leftover auto-continue chatter if we are already waiting on the user.
        if (
            session is not None
            and goal_awaiting_user(session.metadata)
            and turn_continuation.internal_continuation_inbound(msg.metadata)
        ):
            logger.info(
                "Suppressed outbound while goal awaits user for {}:{}",
                msg.channel,
                msg.sender_id,
            )
            return None
    ...
    async def _state_restore(self, ctx: TurnContext) -> TurnState:
        ...
        # Real user input resumes a goal that was paused waiting for a choice.
        if not turn_continuation.internal_continuation_inbound(msg.metadata):
            if set_goal_awaiting_user(ctx.session.metadata, False):
                self.sessions.save(ctx.session)
            self._cancel_goal_await_timer(ctx.session_key)
        elif goal_awaiting_user(ctx.session.metadata):
            # Auto-continue fired while we are waiting on the user — drop it
            # entirely so the chat stays quiet until they reply / pick a chip.
            logger.info(
                "Skipping internal continuation for {}; goal awaits user",
                ctx.session_key,
            )
    ...
    async def _state_run(self, ctx: TurnContext) -> str:
        ...
        # Once we are waiting on the user, do not stream further tokens this turn
        # (covers mid-turn pause after message-tool options were already sent).
        base_on_stream = ctx.on_stream
        if base_on_stream is not None:
            async def _gated_on_stream(delta: str) -> None:
                if ctx.session is not None and goal_awaiting_user(ctx.session.metadata):
                    return
                await base_on_stream(delta)

            ctx.on_stream = _gated_on_stream
        ...
        # Any user-visible reply (or message-tool delivery) while a sustained goal
        # is open means we wait for the human — do not auto-continue / re-ask.
        if ctx.session is not None and sustained_goal_active(ctx.session.metadata):
            message_tool = self.tools.get("message")
            message_sent = isinstance(message_tool, MessageTool) and bool(
                message_tool._sent_in_turn
            )
            filler = is_goal_idle_wait_filler(ctx.final_content)
            spoke = bool((ctx.final_content or "").strip())
            if message_sent or filler or spoke:
                if set_goal_awaiting_user(ctx.session.metadata, True):
                    self.sessions.save(ctx.session)
                self._arm_goal_await_timer(
                    ctx.session_key,
                    channel=ctx.msg.channel,
                    chat_id=ctx.msg.chat_id,
                    metadata=ctx.msg.metadata,
                ...
                if filler:
                    # Do not show or keep prompting with idle-wait narration.
                    ctx.suppress_response = True
                    ctx.final_content = ""
    ...
    async def _state_save(self, ctx: TurnContext) -> str:
        ...
        # Stay silent for idle-wait filler only. Marking awaiting_user after a
        # normal reply must not swallow that reply — it only blocks auto-continue.
        if ctx.session is not None and is_goal_idle_wait_filler(ctx.final_content):
            if set_goal_awaiting_user(ctx.session.metadata, True):
                self.sessions.save(ctx.session)
            ctx.suppress_response = True
            ctx.final_content = ""
            ...
            and goal_awaiting_user(ctx.session.metadata)
            and turn_continuation.internal_continuation_inbound(ctx.msg.metadata)
        ):
            ctx.suppress_response = True
            ctx.final_content = ctx.final_content or ""
        elif (
