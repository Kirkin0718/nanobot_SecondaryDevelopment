...
class AgentRunSpec:
    ...
    # Default 0: after a plain-text yield, stop and wait for the user (no option spam).
    # Tests / explicit autonomous loops may raise this.
    max_goal_continues: int = 0
...
class AgentRunner:
    ...
    async def _try_drain_injections(
        ...
        *,
        ...
        Returns ``(should_continue, updated_cycles, updated_goal_continue_count)``.

        If injections are found and we haven't exceeded _MAX_INJECTION_CYCLES,
        append them to *messages* (and emit a checkpoint if *assistant_message*
        and *iteration* are both provided) and return so the caller continues
        the iteration loop.
        """
        injections: list[dict[str, Any]] = []
        ...
        if (
            ...
            and goal_continue_count < max(0, int(spec.max_goal_continues))
        ):
            predicate = spec.goal_active_predicate
            if predicate is not None and predicate():
                content = self._resolve_goal_continue_content(spec)
                if content is not None:
                    injections = [build_goal_continue_message(content)]
                    goal_continue_count += 1
        if not injections:
            return False, injection_cycles, goal_continue_count
        if real_injection:
            injection_cycles += 1
        ...
        return True, injection_cycles, goal_continue_count

    def _resolve_goal_continue_content(self, spec: AgentRunSpec) -> str | None:
        """Return continuation prompt text, or None to skip (wait for the user)."""
        ...
        if isinstance(custom, str):
            text = custom.strip()
            return text or None
        return SUSTAINED_GOAL_CONTINUE_PROMPT

    def _build_goal_continue_message(self, spec: AgentRunSpec) -> dict[str, str]:
        return build_goal_continue_message(self._resolve_goal_continue_content(spec))
        ...
        for iteration in range(spec.max_iterations):
            ...
            if response.should_execute_tools:
                ...
                if fatal_error is not None:
                    ...
                    await hook.after_iteration(context)
                    should_continue, injection_cycles, goal_continue_count = await self._try_drain_injections(
                ...
                _drained, injection_cycles, goal_continue_count = await self._try_drain_injections(
                    spec, messages, None, injection_cycles,
            ...
            drained_after_max_iterations, injection_cycles, goal_continue_count = await self._try_drain_injections(
                spec, messages, None, injection_cycles,
    ...
    async def _run_tool(
        ...
        block = context.pending_tool_block
        if block is not None:
            context.pending_tool_block = None
            event = {
                "name": tool_call.name,
                "status": "error",
                "detail": "user approval denied",
            }
            if block.stop_turn:
                return block.message + hint, event, None
            return block.message + hint, event, None
        try:
            if tool is not None:
