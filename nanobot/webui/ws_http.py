...
_COACH_VALUES_HEADER = "X-Nanobot-Coach-Values"
_COACH_VALUES_HEADER_MAX_BYTES = 200_000
...
class GatewayHTTPHandler:
    ...
    async def _dispatch_session_routes(self, request: WsRequest, got: str) -> Response | None:
        ...
        m = re.match(r"^/api/sessions/([^/]+)/coach$", got)
        if m:
            return self._handle_session_coach_get(request, m.group(1))

        m = re.match(r"^/api/sessions/([^/]+)/coach/hub$", got)
        if m:
            return self._handle_session_coach_hub(request, m.group(1))

        m = re.match(r"^/api/sessions/([^/]+)/coach/notes$", got)
        if m:
            return self._handle_session_coach_notes(request, m.group(1))

        m = re.match(r"^/api/sessions/([^/]+)/coach/checkin$", got)
        if m:
            return self._handle_session_coach_checkin(request, m.group(1))
    ...
    def _coach_session_context(self, request: WsRequest, key: str) -> tuple[str, Path, dict[str, Any]] | Response:
        if not self.check_api_token(request):
            return _http_error(401, "Unauthorized")
        if self.session_manager is None:
            return _http_error(503, "session manager unavailable")
        ...
        if not _is_websocket_channel_session_key(decoded_key):
            return _http_error(404, "session not found")
        scope = self.workspaces.scope_for_session_key(decoded_key)
        workspace = Path(scope.project_path)
        data = self.session_manager.read_session_file(decoded_key) or {}
        metadata = data.get("metadata") if isinstance(data, dict) else {}
        if not isinstance(metadata, dict):
            metadata = {}
        return decoded_key, workspace, metadata

    def _handle_session_coach_get(self, request: WsRequest, key: str) -> Response:
        ctx = self._coach_session_context(request, key)
        if isinstance(ctx, Response):
            return ctx
        decoded_key, workspace, metadata = ctx
        try:
    ...
    def _handle_session_coach_hub(self, request: WsRequest, key: str) -> Response:
        ctx = self._coach_session_context(request, key)
        if isinstance(ctx, Response):
            return ctx
        _, workspace, _metadata = ctx
        try:
            payload = build_coach_hub_payload(workspace=workspace)
        except CoachError as e:
            return _http_error(e.status, e.message)
        return _http_json_response(payload)

    def _handle_session_coach_notes(self, request: WsRequest, key: str) -> Response:
        ctx = self._coach_session_context(request, key)
        if isinstance(ctx, Response):
            return ctx
        decoded_key, workspace, metadata = ctx
        values = _coach_values_from_request(request)
        if values is None:
            return _http_error(400, "missing coach payload")
        text = values.get("text")
        if not isinstance(text, str):
            return _http_error(400, "text must be a string")
        path = values.get("path")
        if path is not None and not isinstance(path, str):
            return _http_error(400, "path must be a string")
        try:
            payload = save_coach_notes(
                workspace=workspace,
                session_metadata=metadata,
                text=text,
                session_key=decoded_key,
    ...
    def _handle_session_coach_checkin(self, request: WsRequest, key: str) -> Response:
        ctx = self._coach_session_context(request, key)
        if isinstance(ctx, Response):
            return ctx
        _, workspace, metadata = ctx
        values = _coach_values_from_request(request) or {}
        text = values.get("text")
        if text is not None and not isinstance(text, str):
            return _http_error(400, "text must be a string")
        try:
            payload = append_coach_checkin(
                workspace=workspace,
                session_metadata=metadata,
...
def _coach_values_from_request(request: WsRequest) -> dict[str, Any] | None:
    raw = _case_insensitive_header(request.headers, _COACH_VALUES_HEADER)
    if not raw:
        return None
    if len(raw.encode("utf-8")) > _COACH_VALUES_HEADER_MAX_BYTES:
        return None
    try:
        values = json.loads(raw)
    except Exception:
