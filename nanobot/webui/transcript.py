    ...
    *,
    ...
    for idx, rec in enumerate(lines):
        ...
        if ev == "message":
            ...
            raw_buttons = rec.get("buttons")
            if isinstance(raw_buttons, list) and raw_buttons:
                normalized_buttons: list[list[str]] = []
                for row in raw_buttons:
                    if not isinstance(row, list):
                        continue
                    labels = [cell for cell in row if isinstance(cell, str) and cell.strip()]
                    if labels:
                        normalized_buttons.append(labels)
                if normalized_buttons:
                    extra["buttons"] = normalized_buttons
