...
from nanobot.agent.tool_approval import REGISTRY as TOOL_APPROVAL_REGISTRY
from nanobot.agent.tool_approval import ToolApprovalDecision, label_to_decision
...
class WebSocketChannel(BaseChannel):
    ...
    async def _dispatch_envelope(
        ...
        if t == "transcribe_audio":
            ...
            return
        if t == "tool_approval_response":
            cid = envelope.get("chat_id")
            approval_id = envelope.get("approval_id")
            decision_raw = envelope.get("decision")
            if not _is_valid_chat_id(cid) or not isinstance(approval_id, str) or not approval_id.strip():
                await self._send_event(connection, "error", detail="invalid tool_approval_response")
                return
            decision: ToolApprovalDecision | None = None
            if isinstance(decision_raw, str):
                try:
                    decision = ToolApprovalDecision(decision_raw)
                except ValueError:
                    decision = label_to_decision(decision_raw)
            if decision is None:
                await self._send_event(connection, "error", detail="invalid tool_approval decision")
                return
            ok = await TOOL_APPROVAL_REGISTRY.resolve(approval_id.strip(), decision)
            await self._send_event(
                connection,
                "tool_approval_ack",
                chat_id=cid,
        ...
        if t == "message":
            ...
            if content.strip() and await TOOL_APPROVAL_REGISTRY.resolve_latest_for_session(
                session_key,
                content.strip(),
            ):
                return
            ...
            if metadata.get("webui") is True and self.is_allowed(client_id):
                if envelope.get("hidden_history") is True:
                    metadata[HIDDEN_HISTORY_META] = {"kind": "webui_hidden"}
                else:
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        # Inline choice chips for WebUI (same shape as Telegram/Slack keyboards).
